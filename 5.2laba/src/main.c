...
void initialize_semaphores(void) {
    ...
    pthread_cond_init(&free_space, NULL);
}
...
void close_semaphores(void) {
    ...
    pthread_cond_destroy(&free_space);
    pthread_mutex_destroy(&mutex);

    printf("Condition variables and mutexes closed.\n");
}
...
void *producer(void *arg) {
    ...
    do {
        ...
        while (queue->field == queue->size_queue) {
            pthread_cond_wait(&items, &mutex);
        }
